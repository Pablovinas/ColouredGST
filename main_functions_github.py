...
def build_gate(params, pulse_duration, p, markovian_approx = True, intensity_noise = True, representation = 'PTM', phase = 0):
    ...
    #copy the parameters to avoid changing the original array
    params = np.copy(params)

    if markovian_approx:
        params[2:4] = 0

    if not intensity_noise:
        params[4] = 0

    Gamma_1, Delta_1, Gamma_2, Delta_2, DeltaGamma = p * params[0], p * params[1], p * params[2], p * params[3], p * params[4]
    ...
    chi_A = (1 / 4) * ((1 + cmath.exp(-Gamma_1)) * pauli_i + \
        2 * cmath.exp( - (Gamma_1 + DeltaGamma )/ 2) * (cmath.cos(p * pulse_duration) * cmath.cos(theta) + (Delta_1 / (2 * theta)) * cmath.sin(p * pulse_duration) * cmath.sin(theta)) * pauli_z - \
        2 * cmath.exp( - (Gamma_1 + DeltaGamma )/ 2) * (cmath.sin(p * pulse_duration) * cmath.cos(theta) + (Delta_1 / (2 * theta)) * cmath.cos(p * pulse_duration) * cmath.sin(theta)) * pauli_y)

    # Calculate chi_B
    chi_B = (1 / 4) * ((1 - cmath.exp(-Gamma_1)) * pauli_i - \
        cmath.exp( - (Gamma_1 + DeltaGamma )/ 2) * (cmath.sin(theta) / theta) * (Gamma_2 * cmath.cos(p * pulse_duration) + Delta_2 * cmath.sin(p * pulse_duration)) * pauli_z - \
        cmath.exp( - (Gamma_1 + DeltaGamma )/ 2) * (cmath.sin(theta) / theta) * (Gamma_2 * cmath.sin(p * pulse_duration) - Delta_2 * cmath.cos(p * pulse_duration)) * pauli_x)
...
def build_gate_ptm(params, pulse_duration, markovian_approx = True, intensity_noise = True, phase = 0):
    ...
    #parameters
    Gamma_1, Delta_1, Gamma_2, Delta_2, DeltaGamma = params[0], params[1], params[2], params[3], params[4]

    #theta
    theta = 1 / 2 * cmath.sqrt(Delta_1 ** 2 - Delta_2 ** 2 - Gamma_2 ** 2)

    #parts of matrix elements
    sigma_1 = np.real(np.exp(- (Gamma_1 + DeltaGamma)/ 2) * (cmath.cos(theta) * np.cos(pulse_duration) \
                                      + Delta_1 * cmath.sin(theta) * np.sin(pulse_duration) / (2 * theta) ))
    
    sigma_2 = - np.real(np.exp(- (Gamma_1 + DeltaGamma) / 2) * (cmath.cos(theta) * np.sin(pulse_duration) \
                                      + Delta_1 * cmath.sin(theta) * np.cos(pulse_duration) / (2 * theta) ))

    
    sigma_3 = np.real(np.exp(- (Gamma_1 + DeltaGamma) / 2) * cmath.sin(theta) * (- Delta_2 * np.cos(pulse_duration) \
                                                          + Gamma_2 * np.sin(pulse_duration)) / (2 * theta))
    
    sigma_4 = np.real(np.exp(- (Gamma_1 + DeltaGamma) / 2) * cmath.sin(theta) * (Gamma_2 * np.cos(pulse_duration) \
                                                          + Delta_2 * np.sin(pulse_duration)) / (2 * theta))
    #bulid the matrix
    chi = np.zeros((4, 4), 'd')
    chi[0,0] = 1
    ...
    if phase == 0:
        ...
        chi[2,2] = sigma_1 - sigma_4
        chi[3,3] = sigma_1 + sigma_4
        chi[2,3] = sigma_2 - sigma_3
        chi[3,2] = - (sigma_3 + sigma_2)

    elif phase == np.pi / 2:

        chi[2,2] = np.exp(-Gamma_1)
        chi[1,1] = sigma_1 - sigma_4
        chi[3,3] = sigma_1 + sigma_4
        chi[1,3] = sigma_2 - sigma_3
...
def quantum_channel(pulse_duration, omega, taus, cs, p = 1, markovian_approx = True, intensity_noise = True, representation = 'PTM', phase = 0):
    ...
    # Additional parameter for inteinsity noise
    tau_intensity, c_intensity = taus[1], cs[1]
    if not intensity_noise:
        DeltaGamma = 0
    else:
        DeltaGamma = c_intensity * (tau_intensity ** 2) * ((pulse_duration / omega) + (exp(-(pulse_duration / omega) / tau_intensity) - 1) * tau_intensity)
    ...
    gate = build_gate_ptm(np.array([Gamma_1, Delta_1, Gamma_2, Delta_2, DeltaGamma]), pulse_duration, markovian_approx , intensity_noise, phase)

    # Divide by p to return the parameter values
    Gamma_1, Gamma_2, Delta_1, Delta_2, DeltaGamma = Gamma_1 / p, Gamma_2 / p, Delta_1 / p, Delta_2 / p, DeltaGamma / p

    return gate, Gamma_1, Delta_1, Gamma_2, Delta_2, DeltaGamma
...
class gate_set:
    ...
    def __init__(self, params, p, markovian_approx, intensity_noise, representation, physical_gate_set_params):
        ...
        # `intensity_noise` indicates whether intensity noise is considered.
    ...
    def gates(self):
        ...
        interval = [range(5), range(5,10), range(5,10), range(5,10), range(5,10)]
        # Constructs and returns a list of gates by iterating over `physical_gate_set_params` and applying the `build_gate` function.
        # return [build_gate(self.params[interval[i]], self.physical_gate_set_params[i,0], self.p, self.markovian_approx, self.intensity_noise, self.representation, self.physical_gate_set_params[i,1])
        #         for i in range(self.physical_gate_set_params.shape[0])] 

        return [build_gate_ptm(self.params[interval[i]], self.physical_gate_set_params[i,0], self.markovian_approx, self.intensity_noise, self.physical_gate_set_params[i,1])
                 for i in range(self.physical_gate_set_params.shape[0])] 
    ...
    def state(self):
        ...
        return ptm_state(self.params[10:13])
    ...
    def measurement(self):
        ...
        return ptm_state(self.params[13:17])
...
class gate_set_noise_params:
    ...
    def __init__(self, noise_params, state_params, measurement_params, p, markovian_approx, intensity_noise, representation, physical_gate_set_params):
    ...
    def gates(self):
        return [quantum_channel(self.physical_gate_set_params[i,0], self.noise_params[0], self.noise_params[1],
                                self.noise_params[2], self.p, self.markovian_approx, self.intensity_noise,
                                self.representation, self.physical_gate_set_params[i,1])[0]
...
def stochastic_matrices(phis, Mnoise, pulse_duration, intensity_noise, cs, taus, omega):
    ...
    if intensity_noise:
        c_intensity, tau_intensity = cs[1], taus[1]
        exp_neg_dt_tau_intensity = np.exp(-dt / tau_intensity)
        sqrt_term_intensity = np.sqrt(0.5 * c_intensity * tau_intensity * (1 - np.exp(-2 * dt / tau_intensity)))
    else:
        exp_neg_dt_tau_intensity = 0
        sqrt_term_intensity = 0
    ...
    for phi in phis:
        ...
        for _ in range(int(Mnoise)):
            ...
            for k in range(N):
                ...
                w1 = np.random.normal()
                w2 = np.random.normal()
                Xn[k + 1] = Xn[k] * exp_neg_dt_tau + sqrt_term * w1
                Xn_intensity[k + 1] = Xn_intensity[k] * exp_neg_dt_tau_intensity + sqrt_term_intensity * w2

                U = expm(-1j * 0.5 * ((Xn[k]) * sz + (omega + Xn_intensity[k]) * (cos_phi * sx - sin_phi * sy)) * dt) @ U
